...
import math
import pandas as pd
from labels import sub_industry_map, sector_map
...
def match_subIndustry(df) : 
    ...
    for row in df[mask].index : 
        ...
        df.loc[row, 'GICSSubIndustry'] = sub_industry_map[company_subIndustry]
...
def fill_sectors(df, column):
    ...
    for row in df[mask].index : 
        ...
        df.loc[row, column] = sector_map[company_name][column]
